...
class InfrastructureFactory(object):
    ...
    def create_model(config):
        ...
        elif config["system_class"].lower() in [
            "electricpowernetwork",
            "electric power network",
            "electric_power_network",
            "electricitypowernetwork",
            "electricity power network",
        ]:
            return ElectricPowerNetwork(**config)
        elif config["system_class"].lower() in [
            "electricpowernetwork4l",
            "electric power network4l",
            "electric_power_network4l",
            "electricitypowernetwork4l",
            "electricity power network4l",
        ]:
            return ElectricPowerNetwork(**config)
        else:
            return GenericInfrastructure(**config)
...
class Infrastructure(Base):
    ...
    output_nodes = Element(
        "dict", "The components that output from the infrastructure system", dict
    )
    ...
    system_class = None
    system_dmg_states = None
    system_output_capacity = None
    ...
    def __init__(self, **kwargs):
        ...
        self.default_ds_lims = [0.0, 0.05, 0.40, 0.70, 1.00]
        self.ds_lims_compclasses = {}

        self._component_graph = ComponentGraph(self.components)
        self.system_output_capacity = self.get_nominal_output()

    def get_ds_lims_for_compclass(self, component_class):
        """
        Get damage state limits for a component class.
        Returns default limits if component class is not explicitly defined.

        :param component_class: The component class to get limits for
        :return: List of damage state limits
        """
        return self.ds_lims_compclasses.get(component_class, self.default_ds_lims)
    ...
    def calc_output_loss(self, scenario, component_damage_state_ind):
        ...
        if_level_loss = np.zeros((scenario.num_samples, len(self.components)), dtype=float)
        # Infrastructure loss: sum of component loss
        if_level_economic_loss = np.zeros(scenario.num_samples, dtype=float)
        # Component functionality
        if_level_functionality = np.zeros((scenario.num_samples, len(self.components)), dtype=float)
        # output for the level of damage
        if_level_output = np.zeros((scenario.num_samples, len(self.output_nodes)), dtype=float)  # type: ignore
    ...
    def get_nominal_output(self):
        ...
        if not self.if_nominal_output:
            ...
            for output_comp in list(self.output_nodes.values()):  # type: ignore # noqa: E1101
    ...
    def compute_output_given_ds(self, comp_level_func):
        ...
        system_outflows_sample = np.zeros(len(self.output_nodes))  # pyright: ignore[reportArgumentType]
        for output_index, (output_comp_id, output_comp) in enumerate(self.output_nodes.items()):  # type: ignore # noqa: E1101
            # track the outputs by source type (e.g. water or coal)
            total_supply_flow_by_source = {}
            for supply_comp_id, supply_comp in self.supply_nodes.items():  # type: ignore # noqa: E1101
            ...
            estimated_capacity_fraction = min(
                total_available_flow, output_comp["capacity_fraction"]
            )
    ...
    def calc_response(self, component_loss, comp_sample_func, component_damage_state_ind):
        ...
        for comp_index, comp_id in enumerate(component_list_sorted):
            ...
            comp_damage_state_ind_binned[comp_id] = np.bincount(
                component_damage_state_array[:, comp_index], minlength=len(comp_ds_levels)
            )

            comp_resp_dict[(comp_id, "loss_mean")] = np.mean(component_loss[:, comp_index])

            comp_resp_dict[(comp_id, "loss_std")] = np.std(component_loss[:, comp_index])

            comp_resp_dict[(comp_id, "func_mean")] = np.mean(comp_sample_func[:, comp_index])

            comp_resp_dict[(comp_id, "func_std")] = np.std(comp_sample_func[:, comp_index])

            comp_resp_dict[(comp_id, "failure_rate")] = np.mean(
                component_damage_state_ind[:, comp_index] >= (len(component.damage_states) - 1)
            )  # noqa:W503
        ...
        for ct_id in self.get_component_types():
            ...
            ct_pos_index = [list(component_list_sorted).index(x) for x in comps_of_a_type]

            comptype_resp_dict[(ct_id, "loss_mean")] = np.mean(component_loss[:, ct_pos_index])

            comptype_resp_dict[(ct_id, "loss_std")] = np.std(component_loss[:, ct_pos_index])

            comptype_resp_dict[(ct_id, "loss_tot")] = np.sum(
                component_loss[:, ct_pos_index]
            ) / float(num_samples)

            comptype_resp_dict[(ct_id, "func_mean")] = np.mean(comp_sample_func[:, ct_pos_index])

            comptype_resp_dict[(ct_id, "func_std")] = np.std(comp_sample_func[:, ct_pos_index])

            acomponent = self.components[comps_of_a_type[0]]
            comptype_resp_dict[(ct_id, "failure_rate")] = np.mean(
                component_damage_state_ind[:, ct_pos_index] >= (len(acomponent.damage_states) - 1)
            )  # noqa:W503
        # ---------------------------------------------------------------
        ...
        for cls_id in self.get_component_classes():
            comps_of_a_cls = sorted(list(self.get_components_for_class(cls_id)))
            comp_cls_pos_index = [list(component_list_sorted).index(x) for x in comps_of_a_cls]

            comp_cls_dmg_indices_tmp = component_damage_state_array[:, comp_cls_pos_index]

            # Iterate over number of samples
            tmparr = np.zeros(shape=(rows, len(comp_ds_levels)))
            for i in range(rows):
                tmparr[i] = np.bincount(comp_cls_dmg_indices_tmp[i], minlength=len(comp_ds_levels))

            comp_cls_dmg_level_percentages_matrix = 100 * (tmparr / float(len(comps_of_a_cls)))
            ...
            comp_cls_dmg_index_expected[cls_id] = np.max(np.where(tmp == np.max(tmp)))
    ...
    def get_component_types(self):
        ...
        """
        uncosted_comptypes = ["JUNCTION_NODE", "CONN_NODE", "SYSTEM_INPUT", "SYSTEM_OUTPUT"]
    ...
    def get_system_damage_states(self):
        ...
        return self.system_dmg_states

    def get_system_damage_state_bounds(self):
        """
...
class GenericInfrastructure(Infrastructure):
    def __init__(self, **kwargs):
        super(GenericInfrastructure, self).__init__(**kwargs)
        # Define basic uncosted classes, typically used as model artefacts
        ...
        self.type = "GenericInfrastructure"
        self.default_ds_lims = [0.0, 0.05, 0.40, 0.70, 1.00]
        self.ds_lims_compclasses = {}
    ...
    def get_system_damage_states(self):
        """
        Return a list of the damage state labels
        :return: List of strings representing the system damage levels.
        """
        self.system_dmg_states = [
            "DS0 None",
            "DS1 Slight",
    ...
    def get_system_damage_state_bounds(self):
        """
        Return default damage scale boundaries
        :return: Array of real numbers representing damage state boundaries
        """
        return [0.02, 0.1, 0.25, 0.5, 1.0]
...
class Substation(Infrastructure):
    def __init__(self, **kwargs):
        ...
        self.default_ds_lims = [0.05, 0.40, 0.70, 1.00]
        self.ds_lims_compclasses = {
            "Bus": [0.05, 0.40, 0.70, 1.00],
            "Control Building": [0.05, 0.40, 0.70, 1.00],
            "Circuit Breaker": [0.05, 0.40, 0.70, 1.00],
            "Disconnect Switch": [0.05, 0.40, 0.70, 1.00],
            "Current Transformer": [0.05, 0.40, 0.70, 1.00],
            "Voltage Transformer": [0.05, 0.40, 0.70, 1.00],
            "Potential Transformer": [0.05, 0.40, 0.70, 1.00],
            "Autotransformer": [0.05, 0.40, 0.70, 1.00],
            "Power Transformer": [0.05, 0.40, 0.70, 1.00],
            "Surge Protection": [0.05, 0.40, 0.70, 1.00],
            "System Control": [0.05, 0.40, 0.70, 1.00],
            "Measurement Instrumentation": [0.05, 0.40, 0.70, 1.00],
            "Power Factor Correction": [0.05, 0.40, 0.70, 1.00],
            "Lightning Arrester": [0.05, 0.40, 0.70, 1.00],
            "Station Power Supply": [0.05, 0.40, 0.70, 1.00],
        }
    ...
    def get_system_damage_states(self):
        ...
        :return: List of strings detailing the system damage levels.
        """
        self.system_dmg_states = ["DS0 None", "DS1 Minor", "DS2 Extensive", "DS3 Complete"]
        return self.system_dmg_states
...
class PotableWaterTreatmentPlant(Infrastructure):
    def __init__(self, **kwargs):
        ...
        self.type = "PotableWaterTreatmentPlant"
        self.uncosted_classes = [
            "MODEL ARTEFACT",
            "SYSTEM INPUT",
            "SYSTEM OUTPUT",
...
class PotableWaterPumpStation(Infrastructure):
    def __init__(self, **kwargs):
        ...
        # Initiate the water treatment plant values, which have been used
        # in all current testing
        self.type = "PotableWaterPumpStation"
        self.uncosted_classes = [
            "MODEL ARTEFACT",
            "SYSTEM INPUT",
            "SYSTEM OUTPUT",
        ...
        self.default_ds_lims = [0.0, 0.05, 0.40, 0.70, 1.00]
        self.ds_lims_compclasses = {
            "SYSTEM INPUT": [0.0, 0.05, 0.40, 0.70, 1.00],
            "SYSTEM OUTPUT": [0.0, 0.05, 0.40, 0.70, 1.00],
            "Generator": [0.0, 0.05, 0.40, 0.70, 1.00],
            "Basin": [0.0, 0.05, 0.40, 0.70, 1.00],
            "Building": [0.0, 0.05, 0.40, 0.70, 1.00],
            "Chlorination Equipment": [0.0, 0.05, 0.40, 0.70, 1.00],
            "Electric Power Equipment": [0.0, 0.05, 0.40, 0.70, 1.00],
            "Electric Power Supply Commercial": [0.0, 0.05, 0.40, 0.70, 1.00],
            "Electric Power Supply Backup": [0.0, 0.05, 0.40, 0.70, 1.00],
            "Electrical Control Equipment": [0.0, 0.05, 0.40, 0.70, 1.00],
            "Pump": [0.0, 0.05, 0.40, 0.70, 1.00],
            "Pipework": [0.0, 0.05, 0.40, 0.70, 1.00],
            "Chemical Tank": [0.0, 0.05, 0.40, 0.70, 1.00],
            "Concrete Tank": [0.0, 0.05, 0.40, 0.70, 1.00],
            "Steel Tank": [0.0, 0.05, 0.40, 0.70, 1.00],
            "Well": [0.0, 0.05, 0.40, 0.70, 1.00],
        }

    def get_system_damage_state_bounds(self):
        return [0.01, 0.1, 0.25, 0.5, 1.0]
...
class ElectricPowerNetwork(Infrastructure):
    def __init__(self, **kwargs):
        # Initialise the power network class
        super(ElectricPowerNetwork, self).__init__(**kwargs)
        self.type = "ElectricPowerNetwork"
        self.uncosted_classes = [
            "JUNCTION",
            "JUNCTION POINT",
            "JUNCTION NODE",
        ...
        self.default_ds_lims = [0.05, 0.40, 0.70, 1.00]
        self.ds_lims_compclasses = {
            "Distribution Substation": [0.05, 0.40, 0.70, 1.00],
            "Zone Substation": [0.05, 0.40, 0.70, 1.00],
            "Transmission Substation": [0.05, 0.40, 0.70, 1.00],
            "Terminal Substation": [0.05, 0.40, 0.70, 1.00],
            "Power Generation Thermal": [0.05, 0.40, 0.70, 1.00],
            "Power Generation Solar Farm": [0.05, 0.40, 0.70, 1.00],
            "Power Generation Wind Turbine Onshore": [0.05, 0.40, 0.70, 1.00],
            "Power Generation Wind Turbine Offshore": [0.05, 0.40, 0.70, 1.00],
            "Power Generation Geothermal": [0.05, 0.40, 0.70, 1.00],
        }

    def get_system_damage_states(self):
        """
        The list of the damage state labels.
        :return: List of strings detailing the system damage levels.
        """
        self.system_dmg_states = ["DS0 None", "DS1 Minor", "DS2 Extensive", "DS3 Complete"]
        return self.system_dmg_states
...
class ElectricPowerNetwork4L(Infrastructure):
    def __init__(self, **kwargs):
        # Initialise the power network class
        super(ElectricPowerNetwork4L, self).__init__(**kwargs)
        self.type = "ElectricPowerNetwork4L"
        self.uncosted_classes = [
            "JUNCTION",
            "JUNCTION POINT",
            "JUNCTION NODE",
        ...
        self.default_ds_lims = [0.01, 0.25, 0.50, 0.75, 1.00]
        self.ds_lims_compclasses = {
            "Distribution Substation": [0.01, 0.25, 0.50, 0.75, 1.00],
            "Zone Substation": [0.01, 0.25, 0.50, 0.75, 1.00],
            "Transmission Substation": [0.01, 0.25, 0.50, 0.75, 1.00],
            "Terminal Substation": [0.01, 0.25, 0.50, 0.75, 1.00],
            "Power Generation Thermal": [0.01, 0.25, 0.50, 0.75, 1.00],
            "Power Generation Solar Farm": [0.01, 0.25, 0.50, 0.75, 1.00],
            "Power Generation Wind Turbine Onshore": [0.01, 0.25, 0.50, 0.75, 1.00],
            "Power Generation Wind Turbine Offshore": [0.01, 0.25, 0.50, 0.75, 1.00],
            "Power Generation Geothermal": [0.01, 0.25, 0.50, 0.75, 1.00],
        }
    ...
    def get_system_damage_state_bounds(self):
        ...
        """
        # return [0.01, 0.05, 0.40, 0.70, 1.00]
        return [0.01, 0.25, 0.50, 0.75, 1.00]
