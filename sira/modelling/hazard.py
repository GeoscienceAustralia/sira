...
def find_nearest(lon, lat, df, col):
    ...
    """
    distances = df.apply(lambda row: haversine_dist(lon, lat, row["pos_x"], row["pos_y"]), axis=1)
...
class Hazard(object):
    def __init__(
        self, hazard_event_id, hazard_input_method, hazard_intensity_header, hazard_data_df
    ):
        ...
        if str(self.hazard_input_method).lower() in ["hazard_file", "scenario_file"]:
            ...
            if site_id is None:
                hazard_intensity = find_nearest(
                    x_location, y_location, self.hazard_data_df, self.hazard_event_id
                )
            # -----------------------------------------------------------------------
    ...
    def __str__(self):
        return {"Data shape": self.hazard_data_df.shape, "Headers": self.hazard_data_df.columns}
...
class HazardsContainer(object):
    ...
    def __init__(self, configuration, model_file_path):
        ...
        component_table = pd.DataFrame.from_dict(ci_model["component_list"], orient="index")
        ...
        if configuration.HAZARD_INPUT_METHOD.lower() in ["hazard_file", "scenario_file"]:
            ...
            (self.hazard_data_df, self.hazard_scenario_list, self.hazard_intensity_list) = (
                HazardsContainer.populate_scenario_hazard_using_hazard_file(
                    hazard_file_path, self.HAZARD_INPUT_HEADER, self.HAZARD_SCALING_FACTOR
                )
            )
        ...
        # ---------------------------------------------------------------------
            ...
            self.num_hazard_pts = int(
                round(
                    (configuration.INTENSITY_MEASURE_MAX - configuration.INTENSITY_MEASURE_MIN)
                    / float(configuration.INTENSITY_MEASURE_STEP)
                    + 1
                )
            )

            # Using the limits and step generate a list of hazard intensity values
            self.hazard_intensity_list = np.linspace(
    ...
    def populate_scenario_hazard_using_hazard_file(
        ...
        site_header = "site_id"
        dtypes = {site_header: "str", event_header: "str", hazard_header: "float32"}
        ...
        hazard_input_df = hazard_input_df.droplevel(axis="columns", level=0)  # type: ignore
    ...
    def populate_scenario_hazard_using_hazard_array(hazard_intensity_list, hazard_header):
        hazard_scenario_list = ["%0.3f" % float(x) for x in hazard_intensity_list]
