...
class DataGetter(object):
    ...
    class DataContainer(object):
        """
        An empty container for holding data from input file
        """
        pass
    ...
    def __init__(self, setup_file):
        ...
        self.ioconfig = self.DataContainer()
        self.scenario = self.DataContainer()
        self.facility = self.DataContainer()

        self.ioconfig.input_dir_name = self.setup["INPUT_DIR_NAME"]
        self.ioconfig.output_dir_name = self.setup["OUTPUT_DIR_NAME"]
        self.ioconfig.sys_config_file_name = self.setup["SYS_CONF_FILE_NAME"]
        self.ioconfig.input_path = None
        self.ioconfig.output_path = None
        self.ioconfig.raw_output_dir = None

        self.config_io()
        self.assign_scenario_data()
        self.assign_infrastructure_data()

    def config_io(self):
        self.ioconfig.input_path = os.path.join(os.getcwd(), self.ioconfig.input_dir_name)

        if not os.path.exists(self.ioconfig.output_dir_name):
            os.makedirs(self.ioconfig.output_dir_name)

        self.ioconfig.output_path = \
            os.path.join(os.getcwd(), self.ioconfig.output_dir_name)

        self.ioconfig.raw_output_dir = \
            os.path.join(os.getcwd(), self.ioconfig.output_dir_name,
                         'raw_output')

        if not os.path.exists(self.ioconfig.raw_output_dir):
            os.makedirs(self.ioconfig.raw_output_dir)

    def assign_scenario_data(self):
        """
        Assign hazard parameters to scenario to be simulated
        """
        self.scenario.fit_pe_data = self.setup["FIT_PE_DATA"]
        self.scenario.fit_restoration_data = self.setup["FIT_RESTORATION_DATA"]
        self.scenario.save_vars_npy = self.setup["SAVE_VARS_NPY"]
        self.scenario.hazard_transfer_param = self.setup["HAZARD_TRANSFER_PARAM"]
        self.scenario.hazard_transfer_unit = self.setup["HAZARD_TRANSFER_UNIT"]
        self.scenario.haz_param_min = self.setup["PGA_MIN"]
        self.scenario.haz_param_max = self.setup["PGA_MAX"]
        self.scenario.haz_param_step = self.setup["PGA_STEP"]
        self.scenario.num_samples = self.setup["NUM_SAMPLES"]
        self.scenario.time_unit = self.setup["TIME_UNIT"]
        self.scenario.restore_time_step = self.setup["RESTORE_TIME_STEP"]
        self.scenario.restore_pct_chkpoints = self.setup["RESTORE_PCT_CHKPOINTS"]
        self.scenario.restore_time_upper = self.setup["RESTORE_TIME_UPPER"]
        self.scenario.restore_time_max = self.setup["RESTORE_TIME_MAX"]
    ...
    def assign_infrastructure_data(self):
        ...
        """
        self.facility.system_class = self.setup["SYSTEM_CLASS"]
        self.facility.commodity_flow_types = self.setup["COMMODITY_FLOW_TYPES"]
        self.facility.sys_config_file_name = self.setup["SYS_CONF_FILE_NAME"]

        self.facility.sys_config_file = os.path.join(
            os.getcwd(), self.ioconfig.input_dir_name,
            self.facility.sys_config_file_name)

        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        # List of components in the system, with specified attributes
        self.facility.comp_df = pd.read_excel(
            self.facility.sys_config_file, 'comp_list',
            index_col='component_id',
            skiprows=3, skipinitialspace=True)

        self.facility.comp_df = self.facility.comp_df.rename(
        ...
        self.facility.node_conn_df = pd.read_excel(
            self.facility.sys_config_file, 'component_connections',
            index_col=None, skiprows=3, skipinitialspace=True)

        self.facility.node_conn_df = self.facility.node_conn_df.rename(
            columns={'Orig': 'Origin',
                     'Dest': 'Destination',
                     'Capacity': 'Capacity',
                     'Weight': 'Weight',
        ...
        self.facility.sysout_setup = pd.read_excel(
            self.facility.sys_config_file, 'output_setup',
            index_col='OutputNode',
            skiprows=3, skipinitialspace=True)

        self.facility.sysout_setup = self.facility.sysout_setup.rename(
        ...
        self.facility.sysout_setup = self.facility.sysout_setup.sort(
            'Priority', ascending=True)

        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        # Configuration of input nodes
        # i.e. supply of 'commodities' into the system
        self.facility.sysinp_setup = pd.read_excel(
            self.facility.sys_config_file, 'supply_setup',
            index_col='InputNode',
            skiprows=3, skipinitialspace=True)

        self.facility.sysinp_setup = self.facility.sysinp_setup.rename(
        ...
        self.facility.fragility_data = pd.read_excel(
            self.facility.sys_config_file, 'comp_type_dmg_algo',
...
class Scenario(object):
    ...
    def __init__(self, setup_file):
        # self.setup = readfile(setup_file)
        self.data = DataGetter(setup_file).scenario
        self.io = DataGetter(setup_file).ioconfig
        ...
        # Assign default directory paths
        # self.input_path = "."
        # self.output_path = "."
    ...
    def set_io_dirs(self):
        ...
        self.raw_output_dir = \
            os.path.join(os.getcwd(), self.output_dir_name, 'raw_output')
...
class Facility(object):
    ...
    def __init__(self, setup_file):
        # self.setup = readfile(setup_file)
        self.io = DataGetter(setup_file).ioconfig
        self.data = DataGetter(setup_file).facility
...
class ComponentType(object):
    ...
    def __init__(self, comp_type, fragility_data):
        ...
        # These attributes are to set by instances, within the context
        # of the system:
        self.cost_fraction = {}
        self.node_type = {}
        self.node_cluster = {}
