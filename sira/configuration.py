...
class Configuration:
    ...
    def __init__(self, config_path, model_path, output_path=None):
        ...
        if str(config["HAZARD_INPUT_METHOD"]).lower() in ["hazard_array", "calculated_array"]:
            self.HAZARD_INPUT_METHOD = "calculated_array"
        elif str(config["HAZARD_INPUT_METHOD"]).lower() in ["scenario_file", "hazard_file"]:
        ...
        if self.HAZARD_INPUT_METHOD in ["hazard_file", "scenario_file"]:
            ...
            self.HAZARD_INPUT_FILE = str(Path(self.HAZARD_INPUT_DIR, config["HAZARD_INPUT_FILE"]))
        ...
        self.RESTORATION_PCT_CHECKPOINTS = int(config["RESTORATION_PCT_CHECKPOINTS"])
        self.RESTORATION_TIME_MAX = int(config["RESTORATION_TIME_MAX"])
        self.RESTORATION_STREAMS = config["RESTORATION_STREAMS"]

        try:
            self.RESTORATION_TIME_STEP = int(config["RESTORATION_TIME_STEP"])
        except KeyError:
            self.RESTORATION_TIME_STEP = 1

        try:
            self.RESTORATION_TIME_MIN = int(config["RESTORATION_TIME_MIN"])
        except KeyError:
            self.RESTORATION_TIME_MIN = 1
