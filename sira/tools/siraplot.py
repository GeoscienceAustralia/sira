...
font_path = ["./resources/Fonts/Open Sans"]
for font in font_manager.findSystemFonts(font_path):
    font_manager.fontManager.addfont(font)
mpl.rcParams["font.sans-serif"] = ["Open Sans", "Droid Sans", "Tahoma"]
...
def split_long_label(string, delims, max_chars_per_line=20):
    ...
    for i, val in enumerate(splt_str):
        str_list.append(val)
        if len("".join(str_list)) >= max_chars_per_line and (i < len(splt_str) - 1):
            str_list.append("\n")
...
def format_fig(
    ...
    if x_grid:
        axis.xaxis.grid(True, which="major", linestyle="-", linewidth=0.5, color=grid_colr)
    if y_grid:
        axis.yaxis.grid(True, which="major", linestyle="-", linewidth=0.5, color=grid_colr)
