...
def power_calc(fc, sc):
    ...
    component_resp_df = pd.DataFrame(index=mindex, columns=[sc.hazard_intensity_str])
...
def multiprocess_enabling_loop(idxPGA, _PGA_dummy, nPGA, fc, sc):
    ...
    output_array_given_recovery = np.zeros((sc.num_samples, sc.num_time_steps))
    ...
    sys_output_list_given_pga = {k: np.zeros((sc.num_samples,
                                              len(fc.network.out_node_list))) for k in sc.hazard_intensity_str}
    ...
    for i in range(sc.num_samples):
        ...
        for t in range(sc.num_time_steps):
            output_array_given_recovery[i, t] = sum(compute_output_given_ds(cp_func_given_time[:, t], fc))
    ...
    return ids_comp, sys_out_dict, comp_resp_dict, calculated_output_array_single, \
           economic_loss_array_single, output_array_given_recovery
    ...
    if parallel_or_serial:
        ...
        for idxPGA, _PGA in enumerate(sc.hazard_intensity_str):
            ids_comp_vs_haz[_PGA], sys_output_dict[_PGA], component_resp_dict[_PGA], \
                ...
                output_array_given_recovery[:, idxPGA, :] = multiprocess_enabling_loop(
                idxPGA=idxPGA, _PGA_dummy=_PGA, nPGA=sc.num_hazard_pts, fc=fc, sc=sc)

    return ids_comp_vs_haz, sys_output_dict, component_resp_dict, calculated_output_array, \
           economic_loss_array, output_array_given_recovery


def post_porcessing(fc, sc, ids_comp_vs_haz, sys_output_dict, component_resp_dict, calculated_output_array,
                    economic_loss_array, output_array_given_recovery):
    idshaz = os.path.join(sc.output_dir_name, 'ids_comp_vs_haz.pickle')
    ...
    outfile_sysoutput = os.path.join(sc.output_path, 'system_output_vs_hazparam.csv')
    ...
    comptype_resp_df = pd.DataFrame(index=mindex, columns=[sc.hazard_intensity_str])
    ...
    cp_classes_in_system = list(np.unique(fc.comp_df['component_class'].tolist()))
    ...
    if fc.system_class == 'Substation':
        ...
        comp_class_failures = {cc: np.zeros((sc.num_samples, sc.num_hazard_pts)) for cc in cp_classes_costed}
            ...
            for i in range(sc.num_samples):
                for compclass in cp_classes_costed:
                    for c in cp_class_map[compclass]:
                        comp_class_failures[compclass][i, j] += ids_comp_vs_haz[PGA][i, sc.nodes_all.index(c)]
        ...
        np.save(os.path.join(sc.raw_output_dir, 'pe_sys_cpfailrate.npy'), pe_sys_cpfailrate)
    ...
    exp_damage_ratio = np.zeros((len(fc.network.nodes_all), sc.num_hazard_pts))
    for l, PGA in enumerate(sc.hazard_intensity_vals):
        ...
        for j, comp_name in enumerate(fc.network.nodes_all):
            pb = pe2pb(cal_pe_ds(comp_name, PGA, fc.compdict, fc.fragdict, sc))
    ...
    for j in range(sc.num_hazard_pts):
        cpower = \
            np.mean(output_array_given_recovery[:, j, :], axis=0)/fc.nominal_production
    ...
    if sc.save_vars_npy:
        np.save(os.path.join(sc.raw_output_dir, 'economic_loss_array.npy'),
                economic_loss_array)
        np.save(os.path.join(sc.raw_output_dir, 'calculated_output_array.npy'),
                calculated_output_array)
        np.save(os.path.join(sc.raw_output_dir, 'output_array_given_recovery.npy'),
                output_array_given_recovery)
        np.save(os.path.join(sc.raw_output_dir, 'exp_damage_ratio.npy'),
                exp_damage_ratio)
        np.save(os.path.join(sc.raw_output_dir, 'sys_frag.npy'),
                sys_frag)
        np.save(os.path.join(sc.raw_output_dir, 'required_time.npy'),
                required_time)
