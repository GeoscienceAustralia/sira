...
from sira.tools.convert_excel_files_to_json import read_excel_to_json, update_json_structure
...
def ingest_model(config):
    ...
    if extension == "json":
        ...
        json_obj = json.loads(
            read_excel_to_json(config.INPUT_MODEL_PATH), object_pairs_hook=OrderedDict
        )
...
def read_model_from_json(config, model):
    ...
    for component_id in component_list:
        ...
        system_components[component_id] = Component(config, **component_values)
    ...
    for index in node_conn_df:
        ...
        system_component.destination_components[node_conn_df[index]["destination"]] = (
    ...
    for key in component_list:
        for damages_state in component_list[key]["damages_states_constructor"]:
            if damages_state not in infrastructure_system_constructor["system_dmg_states"]:
                infrastructure_system_constructor["system_dmg_states"].append(damages_state)
