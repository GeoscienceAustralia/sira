...
class TestReadingExcelFile(ut.TestCase):
    ...
    def test_folder_structure(self):
        ...
        self.assertTrue(os.path.isdir(os.path.join(self.project_root_dir, "models")), "core models folder not found at " + self.project_root_dir + "!")
        self.assertTrue(os.path.isdir(os.path.join(self.project_root_dir, "tests", "models")), "test models folder not found at " + self.project_root_dir + "!")

        self.assertTrue(os.path.isdir(os.path.join(self.project_root_dir, "simulation_setup")), "core simulation setup folder not found at " + self.project_root_dir + "!")
    ...
    def test_model_files_exists(self):
        ...
        for model_file in self.model_xlsx_files:
            self.assertTrue(os.path.isfile(model_file), "Model excel file not found on path  at" + model_file + " !")
    ...
    def test_reading_data_from_comp_type_dmg_algo(self):
        ...
        required_col_names = ['is_piecewise', 'damage_function', 'damage_ratio', 'functionality', 'recovery_mean', 'recovery_std', 'recovery_95percentile']

        for model_file in self.model_xlsx_files:
            comp_type_dmg_algo = pd.read_excel(model_file, sheet_name='comp_type_dmg_algo', index_col=[0, 1, 2],
                                               header=0, skiprows=0, skipinitialspace=True)
            ...
            possible_values_of_damage_function = ["StepFunc", "LogNormalCDF", "Lognormal", "NormalCDF", "ConstantFunction", "Level0Response", "Level0Recovery", "PiecewiseFunction", "RecoveryFunction"]
