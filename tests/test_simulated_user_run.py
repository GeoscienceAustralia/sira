...
input_files_missing = [("_", "test_missing_inputdir", "-s")]

# ------------------------------------------------------------------------------


@pytest.mark.modelrun
@pytest.mark.parametrize("dir_setup, model_name, run_arg", testdata, indirect=["dir_setup"])
def test_run_model(dir_setup, model_name, run_arg):
    ...
    cmd = [sys.executable, "-m", "sira", "-d", str(inputdir), run_arg]
...
@pytest.mark.parametrize(
    "dir_setup, model_name, run_arg", files_with_incorrect_names, indirect=["dir_setup"]
)
def test_catch_improper_inputfilename(dir_setup, model_name, run_arg):
    ...
    print("*" * 80)
    print(f"inputdir: {str(inputdir)}")
    print(f"str(code_dir): {str(code_dir)}")
...
@pytest.mark.parametrize(
    "dir_setup, model_name, run_arg", input_files_missing, indirect=["dir_setup"]
)
